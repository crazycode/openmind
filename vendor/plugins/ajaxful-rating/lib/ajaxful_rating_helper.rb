module AjaxfulRating # :nodoc:
  module Helper
    ...
    # * <tt>:link_class_prefix</tt> Prefix for the li a CSS class. Default is
    #   'stars'.
    # * <tt>:small_stars</tt> Set this param to true to display smaller images.
    #   Default is false.
    # * <tt>:small_star_class</tt> CSS class for the list when using small
    #   images. Default is 'small-stars'.
    # * <tt>:html</tt> Hash of options to customise the ul tag.
    # * <tt>:remote_options</tt> Hash of options for the link_to_remote
    #   function. Default is {:method => :post, :url =>
    #   rate_rateablemodel_path(rateable)}.
    # * <tt>:wrap</tt> Whether the star list is wrapped within a div tag or not.
    ...
    #
    # The user passed here will *not* be the one who submits the rate. It will
    # be used only for the display behavior of the stars. Like for example, if
    # there is a user logged in or if the current logged in user is able to
    ...
    # I18n:
    #
    # You can translate the title of the images (the tool tip that shows when
    # the mouse is over) and the 'Currently x/x stars' string by setting these
    # keys on your translation hash:
    #
    #   ajaxful_rating:
    #     stars:
    ...
    # Call this method <strong>within head tags</strong> of the main layout to
    # yield the dynamic styles. It will include the necessary stlyesheet and
    ...
    def current_average(rateable)
      ...
      "Current rating: #{rateable.rate_average(true, options[:dimension])}/#{rateable.class.max_rate_value}"
    end
